...
**Estimación de la pose:** Esta fallaba debido a una interpretación incorrecta del álgebra en transformación homogénea, es decir, intenté proyectar la posición del robot de manera lineal utilizando funciones trigonométricas directas sobre el vector de traslación del vector, sin tener en cuenta que puntos estaban expresados en el sistema respecto de la cámara y cuales respecto del mundo real.

**Localización incremental por odometría:** En mi implemnetación anterior cometí un error crítico al igualar directamnete la pose estimada a la lectura del simulador, para luego aplicar un filtro complementario cuando reaparecía el AprilTag.
Esta aproximación rompía la continuidad geométrica, al no calcular la diferencial real del movimiento. 

Esto he tratado de subsanarlo separando la localización en dos estados diferenciados:
Cuando el robot detecta una baliza, calcula una pose absoluta mediante la inversión de la matriz homogénea y la composición de transformaciones de cuerpo rígido (T mundo_cam = T mundo_tag * T tag_cam), almacenando esta lectura odometrica respecto de una referencia para poder calcular los incrementos relativos, esto será relevante más adelante. 
...
Buscamos recorrer el espacio completo de trabajo del robot, y para ello debemos esta constantemente buscando detectar balizas. 
Por otro laod, calculamos el error de alineación en píxeles entre el centro de la imagen y el centroide horizontal del AprilTag.
